...
function HeroVisual() {
  return (
    <div className="relative mx-auto aspect-[.86] w-full max-w-[470px] overflow-hidden rounded-[2.2rem] bg-primary p-4 shadow-[var(--shadow-soft)] sm:p-6" aria-label="Abstract illustration of a bright, welcoming dental office" role="img">
      ...
      <div className="relative flex h-full flex-col justify-between rounded-[1.7rem] border border-primary-foreground/20 bg-primary-foreground/10 p-5 text-primary-foreground backdrop-blur-sm sm:p-7">
        <div className="flex items-start justify-between">
          <span className="rounded-full bg-accent px-3 py-1.5 text-[10px] font-bold uppercase tracking-[.14em] text-accent-foreground">Care, close to home</span>
        ...
        <div>
          <div className="mb-8 space-y-2">
            {[
              ['01', 'Family dentistry', 'A steady place for every age'],
              ['02', 'Cosmetic care', 'Small changes, your smile'],
              ['03', 'Same-day crowns', 'When your day cannot wait'],
            ].map(([number, title, detail], index) => (
              <div key={title} className="flex items-center gap-3 rounded-xl border border-primary-foreground/15 bg-background/10 px-3 py-3">
                <span className={`grid size-8 shrink-0 place-items-center rounded-lg text-[10px] font-bold ${index === 1 ? 'bg-accent text-accent-foreground' : 'bg-background/15 text-primary-foreground'}`}>{number}</span>
                <span className="min-w-0"><strong className="block text-sm">{title}</strong><span className="block truncate text-xs text-primary-foreground/60">{detail}</span></span>
              </div>
            ))}
          </div>
          <p className="font-display text-4xl leading-none sm:text-5xl">A calmer<br /><em>kind</em> of care.</p>
          <p className="mt-4 max-w-[250px] text-sm leading-6 text-primary-foreground/70">Modern dentistry with room for the human part.</p>
        </div>
      </div>
    </div>
  );
    ...
    <section className="container-wide py-20 sm:py-28">
      ...
      <div className="mt-12 divide-y divide-border border-y border-border">
        {services.slice(0, 6).map((service, index) => {
          ...
          return <Link href="/services" key={service.title} data-testid={`card-service-${index}`} className="focus-ring group grid gap-4 bg-background py-5 transition-colors hover:bg-secondary/35 sm:grid-cols-[auto_1fr_auto] sm:items-center sm:gap-6 sm:py-6"><span className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-secondary text-primary"><Icon className="size-5" /></span><span className="font-mono text-[11px] text-muted-foreground">0{index + 1}</span></span><span><h3 className="text-lg font-bold">{service.title}</h3><p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">{service.text}</p></span><ArrowRight className="size-5 text-primary transition-transform group-hover:translate-x-1" /></Link>;
        })}
      </div>
    </section>
  );
    ...
    <section className="bg-secondary/55 py-20 sm:py-28">
      <div className="container-wide grid gap-12 lg:grid-cols-[.78fr_1.22fr] lg:items-center">
        ...
        <div className="flex flex-wrap gap-4 lg:col-start-2">
          <Link href="/about" data-testid="link-home-doctors" className="focus-ring inline-flex min-h-11 items-center gap-2 text-sm font-bold text-primary">Meet the doctors <ArrowRight className="size-4" /></Link>
          <a href="tel:7075528195" data-testid="link-home-doctor-call" className="focus-ring inline-flex min-h-11 items-center gap-2 text-sm font-bold text-primary">Talk with our team <Phone className="size-4" /></a>
        </div>
      </div>
    </section>
  );
...
export function HomePage() {
  return <><PageMeta title="Fairgrounds Dental Practice | Vallejo, CA" description="Warm, modern family and cosmetic dentistry in Vallejo. New patients welcome, most PPO insurance accepted, and same-day emergency visits available." path="/" /><main>
    <section className="relative overflow-hidden"><div className="container-wide grid min-h-[640px] gap-12 py-12 sm:py-20 lg:grid-cols-[1.04fr_.96fr] lg:items-center lg:gap-16 lg:py-20"><div className="relative z-10 animate-rise"><p className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-secondary px-3.5 py-2 text-xs font-bold uppercase tracking-[.14em] text-primary"><span className="size-2 rounded-full bg-accent" /> Fairgrounds Dental · Vallejo, CA</p><h1 className="font-display mt-6 max-w-3xl text-[3.45rem] leading-[.9] tracking-[-.04em] text-foreground sm:text-6xl lg:text-[5.8rem]">Family, cosmetic &amp; urgent dental care <em className="text-primary">close to home.</em></h1><p className="mt-6 max-w-xl text-base leading-7 text-muted-foreground sm:text-lg">A warm, modern dental practice for Vallejo families — from exams and cleanings to cosmetic care, emergency visits, and same-day crowns.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><ButtonLink href="/contact">Request appointment</ButtonLink><a href="tel:7075528195" data-testid="link-home-call" className="focus-ring inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-primary/25 bg-background px-5 py-3.5 text-sm font-bold text-primary transition-colors hover:bg-secondary"><Phone className="size-4" /> Call 707-552-8195</a><a href="tel:7075528195" data-testid="link-home-emergency" className="focus-ring inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-accent/50 bg-accent/20 px-5 py-3.5 text-sm font-bold text-foreground transition-colors hover:bg-accent/35"><CircleAlert className="size-4 text-destructive" /> Dental emergency?</a></div><div className="mt-5 flex flex-wrap gap-x-4 gap-y-2 text-xs font-semibold text-muted-foreground"><span>New patients welcome</span><span className="text-accent">•</span><span>Most PPO insurance accepted</span><span className="text-accent">•</span><span>Cherry financing available</span></div></div><div className="animate-rise lg:justify-self-end" style={{ animationDelay: '.12s' }}><HeroVisual /></div></div><div className="pointer-events-none absolute -bottom-32 -left-24 size-72 rounded-full border-[45px] border-accent/15" /></section>
...
export function NewPatientsPage() {
  ...
  return <><PageMeta title="New Patients | Fairgrounds Dental Practice" description="New to Fairgrounds Dental Practice? Learn about insurance, Cherry financing, your first visit, and how to request an appointment." path="/new-patients" /><main><PageIntro eyebrow="Welcome in" title="Your first visit should feel simple." body="We’ll help with the practical details and the human ones, from insurance questions to knowing what to expect when you take a seat." /><section className="container-wide grid gap-12 py-20 sm:py-28 lg:grid-cols-[.8fr_1.2fr]"><div><SectionHeading eyebrow="Before you arrive" title="A little prepared. Never over-prepared." body="We’ll meet you where you are and make space for a real conversation about your health, comfort, and goals." /><div className="mt-8 space-y-5">{['Share your goals and health history', 'Meet your care team and ask anything', 'Leave with a clear next step'].map((step, i) => <div key={step} className="flex gap-4"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-accent text-sm font-bold">{i + 1}</span><div><h3 className="font-bold">{step}</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">{['We’ll review the details that help us care for you well.', 'We’ll explain what we’re looking at and why it matters.', 'If treatment is needed, we’ll talk through options together.'][i]}</p></div></div>)}</div><div className="mt-8 flex flex-col items-start gap-3 sm:flex-row sm:items-center"><a href="tel:7075528195" data-testid="link-new-patient-call" className="focus-ring inline-flex min-h-11 items-center gap-2 text-sm font-bold text-primary"><Phone className="size-4" /> Questions? Call 707-552-8195</a><ButtonLink href="/contact" secondary>Request a visit</ButtonLink></div></div><div className="rounded-2xl border border-border bg-card p-6 sm:p-8"><p className="text-xs font-bold uppercase tracking-[.18em] text-primary">Common questions</p><div className="mt-5 divide-y divide-border">{faqs.map((faq, i) => <div key={faq.q}><button type="button" aria-expanded={open === i} data-testid={`button-faq-${i}`} onClick={() => setOpen(open === i ? null : i)} className="focus-ring flex min-h-14 w-full items-center justify-between gap-4 py-5 text-left text-sm font-bold">{faq.q}<ChevronDown className={`size-4 shrink-0 text-primary transition-transform ${open === i ? 'rotate-180' : ''}`} /></button>{open === i && <p className="animate-fade max-w-xl pb-5 pr-6 text-sm leading-6 text-muted-foreground">{faq.a}</p>}</div>)}</div></div></section><section className="bg-foreground py-12 text-background sm:py-16"><div className="container-wide grid gap-6 sm:grid-cols-3 sm:gap-8"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-accent">Insurance</p><p className="mt-2 text-sm leading-6 text-background/70">Most PPO plans accepted, including Delta Dental PPO, MetLife, Humana, Guardian, Cigna, Zelis, and Argus.</p></div><div><p className="text-xs font-bold uppercase tracking-[.16em] text-accent">Financing</p><p className="mt-2 text-sm leading-6 text-background/70">Cherry financing is available for eligible care. Ask our team about your options.</p></div><div><p className="text-xs font-bold uppercase tracking-[.16em] text-accent">Urgent needs</p><p className="mt-2 text-sm leading-6 text-background/70">Same-day emergency appointments are available. Call <a href="tel:7075528195" className="font-bold text-accent underline underline-offset-4">707-552-8195</a>.</p></div></div></section><section className="bg-secondary/55 py-20 sm:py-28"><div className="container-wide grid gap-10 lg:grid-cols-[.85fr_1.15fr]"><div><SectionHeading eyebrow="Let’s make a plan" title="Start with a conversation." body="Send a request and our team will follow up during office hours. Or call if you’d rather talk it through." /></div><div className="rounded-2xl border border-border bg-card p-6 sm:p-8"><AppointmentForm /></div></div></section></main></>;
}
